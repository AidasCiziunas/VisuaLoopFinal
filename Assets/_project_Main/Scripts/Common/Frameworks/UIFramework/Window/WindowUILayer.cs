...
namespace SyedAli.Main
{
    ...
    public class WindowUILayer : AUILayer<IWindowController>
    {
        ...
        public bool IsThisWindowOpened(ScreenIds screenId)
        {
            for (int i = 0; i < windowHistory.Count; i++)
            {
                WindowHistoryEntry entry = windowHistory.ElementAt(i);
                if (entry.Screen.ScreenId == screenId.ToString() && entry.Screen.IsVisible == true)
                    return true;
            }
            return false;
        }

        public bool IsThisWindowTopOfWindows(IWindowController windowController)
        {
            if (windowHistory.Peek().Screen == windowController)
            {
                return true;
            }
            else
        ...
        public IWindowController GetTopOfWindows()
        {
            return windowHistory.Peek().Screen;
        }
