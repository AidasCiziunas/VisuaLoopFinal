...
namespace SyedAli.Main
{
    ...
    public class UISettings : ScriptableObject
    {
        ...
        [Tooltip("In case if first top entry screen needs panel to properly display in that case from the controller code we call for second entry to show so that first screen properly appear with panel ")]
        [SerializeField] private bool secondEntryIsPanel = false;

        public bool SecondEntryIsPanel { get => secondEntryIsPanel; }
        ...
        {
            return screensToRegister[0].name;
        }

        public string GetSecondScreenName()
        {
            try
            {
                return screensToRegister[1].name;
            }
            catch
            {
                SmallModuleSignals.Get<DebuggingSignal.LogStack>().Dispatch("No Second Screen Found", this);
                return null;
            }
        }
